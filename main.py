...
if selected == 'Diabetes Management':
    ...
    if st.button('Diabetes Test Result', key='diabetes_button'):
        ...
        # Remove the validation code from here
...
if selected == 'Heart Disease Management':
    ...
    if st.button('Heart Disease Test Result', key='heart_button'):
        ...
        # Validate the inputs (You can add your custom validation here)
        # For simplicity, assuming inputs are valid (you can replace this with your actual validation logic)
        valid_hd = True  # Replace with actual validation logic
        ...
        if valid_hd:
            ...
            if heart_prediction[0] == 1:
                ...
                *Health Recommendations for Heart Disease:*
                ...
                categories = [
                    categorize_heart_risk(float(age), 18, 70),
                    categorize_heart_risk(float(sex), 0, 1),
                    categorize_heart_risk(float(cp), 0, 1),
                    categorize_heart_risk(float(trestbps), 90, 130),
                    categorize_heart_risk(float(chol), 125, 275),
                    categorize_heart_risk(float(fbs), 0, 1),
                    categorize_heart_risk(float(restecg), 0, 1),
                    categorize_heart_risk(float(thalach), 70, 155 ),
                    categorize_heart_risk(float(exang), 0, 1),
                    categorize_heart_risk(float(oldpeak), 0, 1.8 ),
                    categorize_heart_risk(float(slope), 0, 1),
                    categorize_heart_risk(float(ca), 0, 1),
                    categorize_heart_risk(float(thal), 0, 2)
                ]
                
                # Visualize with risk categories
                visualize_heart_disease_risk(risk_factors, user_input_hd, categories, "Heart Disease Risk Factors")
            else:
